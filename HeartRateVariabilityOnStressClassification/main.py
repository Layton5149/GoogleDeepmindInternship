...
#import all hr stats
def splitPPGSignals(base_path, fs, length):
    endArray = []
    endClassifierArray = []
    segmentTranslator = fs*length
    for i in range(1,21):
        print (base_path)
        file_path = (f"{base_path}{i}/BVP.csv")
        ...
        print (len(segmentArray))
        print (len(segmentArray[3]))
...
def main():
    ...
    PPGSignalSegmentArray = splitPPGSignals(file_path_base, fs, length)[0] # use this to extract features about each segment
    PPGsignalsegmentClassifierArray = splitPPGSignals(file_path_base, fs, length)[1] # this is the classifier for stress (1 or 0) for each of the segments
